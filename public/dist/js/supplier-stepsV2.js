...
function validateForm() {
  ...
  for (i = 0; i < currentTabRequiredInputs.length; i++) {
    ...
    else if ($(currentTabRequiredInputs[i]).hasClass('validate-Tax-id-number-and-value-add-registeration-number2')) { // if input is value_add_registeration_number
      if (!validateTax_id_numberAndValue_add_registeration_number2($(currentTabRequiredInputs[i])))
        valid = false;
    }
...
// validate Tax id number and value add registeration number2
$('.validate-Tax-id-number-and-value-add-registeration-number2').on('focusout', function (e) {
  validateTax_id_numberAndValue_add_registeration_number2($(this))
});
...
function validateTax_id_numberAndValue_add_registeration_number2(that) {
  return validate(that, /^[\d]{6}[\d]{5}[\d]{3}$/, 'Tax-id-number-and-value-add-registeration-number2');
}
